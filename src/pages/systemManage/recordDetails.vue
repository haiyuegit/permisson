...
<script>
            ...
            itemList:[
                {
                    ...
                    model:'receiveType',
                    options:[
                        {code:"XJHB_LJ",name:'立即领取型现金红包'},
                        {code:"XJHB_DQ",name:'到期领取行现金红'}
                    ],
                },{
                    label:"发放类型",
                    type:"select",
    ...
    methods: {
        ...
        async enable(params,status){
            ...
            let res;
            if(status){
                res = await this.$service.cashBonusEnable(params)
            }else{
                res = await this.$service.withdrawVoucherDisable(params)
            }
        ...
        async revoke(prizeIdList){
            let res;
            if(prizeIdList){
                res = await this.$service.cashBonusCancel(prizeIdList)
            }
            if(res){
                this.getXjhbList(this.params);
                this.$message.success('改变状态成功');
        ...
        async getXjhbList(params){
            this.loading = true;
            let req = {activeId:this.$route.query.activeId,...params,...this.searchData}
            let res = await this.$service.cashBonusList(req)
