<template>
    <div class="content">
        <topWrap>
            <systemSearch :itemLiitemListst="itemList" isSystem="show" v-on:search="search"></systemSearch>
        </topWrap>
...
export default {
    ...
    methods: {
        async getList(params){
            ...
            let res = await this.$service.totalList(req)
        ...
        handleClick(row) {//查看
            console.log(row)
            //this.$router.push({path: '/systemManage/recordDetails',query:{activeId:row.activeId, activeTopic:row.activeTopic, pageNum: 1, pageSize: this.pageSize, timeType: "giveTime"}})
            this.$router.push({name: 'RECORD_DETAILS',query:{activeId:row.activeId, activeTopic:row.activeTopic, startTime:row.startTime, endTime:row.endTime, pageNum: 1, pageSize: this.pageSize, timeType: "giveTime",}})
        },
        search(data){
            console.log(data)
