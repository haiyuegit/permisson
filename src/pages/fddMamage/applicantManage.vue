...
import obtn from '../../components/applicantManage/operateBtn'
import xdetail from '../../components/applicantManage/viewDialog'
import xadd from '../../components/applicantManage/add'
...
export default {
    ...
    methods: {
        ...
        //新增数据请求接口
         async fddadd(params){
             let res = await this.$service.fddAdd(params);
             if(res){
         ...
         async fddaddChange(params){
             let res = await this.$service.fddaddChange(params);
             if(res){
        ...
        async queryDetail(params,num){
            let res = await this.$service.detail({...params,"dfsToken" : this.dfsToken})
            console.log(res)
