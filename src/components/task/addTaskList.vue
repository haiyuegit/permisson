<template>
    <el-form ref="form" :model="form" :rules="rules" label-width="230px" size="medium">
        ...
        <el-form-item label="是否不允许并发作业"><!--验证：必填prop="concurrentExectionDisallowed"-->
        ...
        <el-form-item label="是否启用"><!--验证：必填prop="isEnabled"-->
        ...
        <el-form-item label="作业实例类型">
            <el-select v-model="form.jobInstanceType" placeholder="请选择">
                <el-option
                        v-for="item in jobInstanceTypes"
                        :key="item.code"
                        :label="item.name"
                        :value="item.code">
                </el-option>
        ...
        <el-form-item :label="taskshiliName" prop="jobInstanceName">
        ...
        <el-form-item label="触发失败后的策略">
            <el-select v-model="form.triggerMisfireInstruction" placeholder="请选择">
                <el-option
                        v-for="item in triggerMisfireInstructions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value">
                </el-option>
...
//                triggerName:'',
        ...
        computed: {
            taskshiliName:function(){
                if(this.form.jobInstanceType === "TARGET"){
                    return "作业实例名（包名.类名）"
                }else{
                    return "作业实例名（name in spring）"
                }
            }
        },
        data() {
            return {
                description: '',
                jobInstanceTypes:[
                    {name:"直接或间接实现 Job 接口",code:'TARGET'},
                    {name:"直接或间接实现 Job 接口的类被托管到 Spring",code:'PROXY'}
                ],
                triggerMisfireInstructions:[
                    {value:1,label:"立即再触发一次"},
                    {value:2,label:"延迟到下一个计划再触发"}
                ],
                rules:{
                    /*jobGroup: [
                   ...
                   /*triggerMisfireInstruction: [
                    ...
                    persistJobDataAfterExecution: [
                        { required: true, message: '是否在作业完成后持久化 JobData', trigger: 'blur' }
                    ],
                    description: [
                        { max: 50, message: '长度在50 个以内字符', trigger: 'blur' }
                    ],*/
