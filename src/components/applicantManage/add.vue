<template>
    ...
    <el-form-item label="企业名称" prop="name">
        <el-input v-model="formDate.name" placeholder="请输入企业名称"></el-input>
    </el-form-item>
    <el-form-item label="营业执照" prop="code">
        <el-input v-model="formDate.code" placeholder="请输入营业执照编号"></el-input>
    </el-form-item>
    <el-form-item label="法人姓名" prop="legalName">
        <el-input v-model.number="formDate.legalName" placeholder="请输入企业法人代表人姓名"></el-input>
    </el-form-item>
    <el-form-item label="手机号码" prop="phone">
        <el-input v-model.number="formDate.phone" placeholder="请输入法人代表人的手机号码"></el-input>
    </el-form-item>
    ...
    <el-form-item label="" prop="desc">
     ...
     <el-form-item label="">
       <el-upload
        ...
        <span slot="tip" class="g_server" style="margin-left: 10px;" type="primary"  @click="submitUpload(fileList2,1)">上传图片</span>
        <div slot="tip" class="el-upload__tip">请上传营业执照，只能上传jpg文件</div>
    </el-upload>
    </el-form-item>
     <el-form-item label="">
       <el-upload
         ...
         list-type="picture">
        <el-button id="udload_btn" size="small" type="primary" @click="validation">选择图片</el-button>
        <span slot="tip" class="g_server" style="margin-left: 10px;" type="primary"  @click="submitUpload(fileList3,2)">上传图片</span>
     ...
     <el-form-item label="">
       <el-upload
         ...
         list-type="picture">
        <el-button id="udload_btn" size="small" type="primary" @click="validation">选择图片</el-button>
        <span slot="tip" class="g_server" style="margin-left: 10px;" type="primary"  @click="submitUpload(fileList4,3)">上传图片</span>
...
export default {
  ...
  data() {
    ...
    //自定义验证
     ...
     var validateCode = (rule, value, callback) => {
        if (value === '') {
          ...
          callback(new Error('请输入请输入营业执照编号'));
        } else { 
      ...
      var validatelegalName = (rule, value, callback) => {
        if (value === '') {
          this.removeItem('legalName');   
          callback(new Error('请输入企业法人代表人姓名'));
        } else { 
           if(this.vLength.indexOf('legalName')<=-1){
             this.vLength.push('legalName');    
          }
          callback();
        }
      };     
      var validatePhone = (rule, value, callback) => {
        ...
        if (value === '') {
           ...
           callback(new Error('请输入企业法人代表人电话号码'));
    ...
    return {
       ...
       legalName: [
                   {validator: validatelegalName, trigger: 'change',type:'string' }
                  ], 
           phone: [
            ...
            if (valid) {
               ...
               // this.$emit("submitAddPerm",params)
                ...
                if(this.isSubmit == true){
                    ...
                    if(this.files.length <= 2 && this.id == ''){
                        this.$message.error("请上传图片")    
                    }else{
                        if(this.id){
                             let paramsChange = {name:this.formDate.name,mobile:this.formDate.phone,legalName:this.formDate.legalName,idCard:this.formDate.code,email:this.formDate.eMail,addressDetail:this.formDate.desc,address:this.address,addressCode:this.addressNo,files:this.files,dfsToken:this.token,id:this.id} 
                             this.$emit("submitChangePerm",paramsChange)
                        }else{
                            let params = {name:this.formDate.name,mobile:this.formDate.phone,legalName:this.formDate.legalName,idCard:this.formDate.code,email:this.formDate.eMail,addressDetail:this.formDate.desc,address:this.address,addressCode:this.addressNo,files:this.files,dfsToken:this.token} 
                             //console.log(params)
                            this.$emit("submitAddPerm",params)
                            console.log(this.files) 
                        }
